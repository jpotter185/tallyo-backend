...
- `enums/League` — single source of truth for supported leagues (`nfl`, `college-football`, `nhl`, `usa.1`, `fifa.world`) and their capability flags (`supportsYearFilter`, `supportsWeekFilter`, `supportsStandings`, `contextMode` — `"season"` vs `"date"` — `statsProfile`, `teamOrder`, `supportsOdds`, `supportsLiveDetails`). Controllers and services branch on these flags instead of hardcoding per-league logic.
